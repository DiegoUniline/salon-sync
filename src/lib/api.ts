...
export const clients = {
  getAll: (search?: string) => request(`/clients${search ? `?search=${search}` : ''}`),
...
export const services = {
  ...
  createBulk: (services: any[]) => request('/services/bulk', { method: 'POST', body: JSON.stringify({ services }) }),
...
export const products = {
  ...
  updateStock: (id: string, stock: number) => request(`/products/${id}/stock`, { method: 'PATCH', body: JSON.stringify({ stock }) }),
  delete: (id: string) => request(`/products/${id}`, { method: 'DELETE' }),
};

// ============ APPOINTMENTS ============
export const appointments = {
  getAll: (params?: { branch_id?: string; stylist_id?: string; date?: string; start_date?: string; end_date?: string; status?: string }) => {
    const query = new URLSearchParams(params as any).toString();
  ...
  update: (id: string, data: any) => request(`/appointments/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  updateStatus: (id: string, status: string) => request(`/appointments/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  delete: (id: string) => request(`/appointments/${id}`, { method: 'DELETE' }),
};

// ============ SALES ============
export const sales = {
  getAll: (params?: { branch_id?: string; date?: string; start_date?: string; end_date?: string }) => {
    const query = new URLSearchParams(params as any).toString();
...
export const expenses = {
  getAll: (params?: { branch_id?: string; category?: string; date?: string; start_date?: string; end_date?: string }) => {
    const query = new URLSearchParams(params as any).toString();
  ...
  update: (id: string, data: any) => request(`/expenses/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
...
export const inventory = {
  getMovements: (params?: { branch_id?: string; product_id?: string; type?: string; start_date?: string; end_date?: string }) => {
    const query = new URLSearchParams(params as any).toString();
  ...
  addIn: (data: any) => request('/inventory/in', { method: 'POST', body: JSON.stringify(data) }),
  addOut: (data: any) => request('/inventory/out', { method: 'POST', body: JSON.stringify(data) }),
  adjust: (data: any) => request('/inventory/adjustment', { method: 'POST', body: JSON.stringify(data) }),
};

// ============ SHIFTS ============
export const shifts = {
  getAll: (params?: { branch_id?: string; status?: string; date?: string; start_date?: string; end_date?: string }) => {
    const query = new URLSearchParams(params as any).toString();
    return request(`/shifts${query ? `?${query}` : ''}`);
  },
  getOpen: (branch_id: string) => request(`/shifts/open?branch_id=${branch_id}`),
  getById: (id: string) => request(`/shifts/${id}`),
  getSummary: (id: string) => request(`/shifts/${id}/summary`),
  open: (data: any) => request('/shifts/open', { method: 'POST', body: JSON.stringify(data) }),
  close: (id: string, final_cash: number) => request(`/shifts/${id}/close`, { method: 'POST', body: JSON.stringify({ final_cash }) }),
};
...
export const cashCuts = {
  ...
  create: (data: any) => request('/cash-cuts', { method: 'POST', body: JSON.stringify(data) }),
...
export const roles = {
  ...
  update: (id: string, data: any) => request(`/roles/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  duplicate: (id: string) => request(`/roles/${id}/duplicate`, { method: 'POST' }),
  delete: (id: string) => request(`/roles/${id}`, { method: 'DELETE' }),
  getUsersList: () => request('/roles/users/list'),
  assignRole: (data: any) => request('/roles/users/assign', { method: 'POST', body: JSON.stringify(data) }),
  removeRole: (user_id: string) => request(`/roles/users/${user_id}/role`, { method: 'DELETE' }),
};

// ============ SCHEDULES ============
export const schedules = {
  getBranch: (branch_id: string) => request(`/schedules/branch/${branch_id}`),
  updateBranch: (branch_id: string, schedule: any) => request(`/schedules/branch/${branch_id}`, { method: 'PUT', body: JSON.stringify({ schedule }) }),
  getStylist: (stylist_id: string, branch_id?: string) => request(`/schedules/stylist/${stylist_id}${branch_id ? `?branch_id=${branch_id}` : ''}`),
  updateStylist: (stylist_id: string, data: any) => request(`/schedules/stylist/${stylist_id}`, { method: 'PUT', body: JSON.stringify(data) }),
  ...
  createBlocked: (data: any) => request('/schedules/blocked', { method: 'POST', body: JSON.stringify(data) }),
  updateBlocked: (id: string, data: any) => request(`/schedules/blocked/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteBlocked: (id: string) => request(`/schedules/blocked/${id}`, { method: 'DELETE' }),
};
...
export const dashboard = {
  get: (branch_id?: string) => request(`/dashboard${branch_id ? `?branch_id=${branch_id}` : ''}`),
  getReports: (params: { start_date: string; end_date: string; type: string; branch_id?: string }) => {
    const query = new URLSearchParams(params as any).toString();
